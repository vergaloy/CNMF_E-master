...
myFolder = 'C:\Users\SSG Lab\Desktop\motion corrected\batch8';
savefiles=1;
...
for k=1:length(theFiles)
 ...
 parametros= h5info(fullFileName);
...
neuron = Sources2D(); 
nams = {fullFileName};          % you can put all file names into a cell array; when it's empty, manually select files 
...
pars_envs = struct('memory_size_to_use', 120, ...   % GB, memory space you allow to use in MATLAB 
    'memory_size_per_patch', 25, ...   % GB, space for loading data within one patch 
    'patch_dims', [64, 64],...  %GB, patch size 
    'batch_frames', (parametros.Datasets.Dataspace.Size(3)/2));           % number of frames per batch 
  % -------------------------      SPATIAL      -------------------------  %
...
ring_radius = 25;  % when the ring model used, it is the radius of the ring used in the background model.

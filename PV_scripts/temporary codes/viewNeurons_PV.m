function viewNeurons_PV(obj, ind, C2, folder_nm)
...
while and(m>=1, m<=length(ind))
    %% full-frame view
    subplot(221); cla;
    obj.image(obj.Cn.*obj.PNR); %
    Coor = get_contours(obj, 0.5, ind(m));
    cont = Coor{1}; 
    hold on
    plot(cont(1,1:end),cont(2,1:end),'Color', 'red', 'linewidth', 1); 
    axis equal; axis off;
    if ind_del(m)
    ...
    subplot(2,2,3:4);cla;
    if ~isempty(C2)
        plot(t, C2(ind(m), :), 'linewidth', 2); hold on;%*max(obj.A(:, ind(m)))
        plot(t, obj.C(ind(m), :), 'r');
    else
...
if save_img
    ...
    if ~isempty(ind(ind_trim))
        obj.A(:, ind(ind_trim)) = reshape(neuron.PNR.*Amask(:, ind(ind_trim));
        try
